<template>
  <div>
    <section v-for="item in courseList">
      ...
      <div class="content"><p class="p1">{{item.course_overview}}</p>
        <div class="div1">
          <div class="div2"><img
            ...
            alt="">
            <span class="span1">{{item.number_of_students_studied}}</span>
            <span class="span2">{{item.course_difficulty}}</span>
          </div>
...
<script>
  export default {
    ...
    mounted() {
      let that = this;
      this.$axios.request({
        url: 'http://127.0.0.1:8000/linux_course/',
        method: 'get',
      }).then(function (data) {
