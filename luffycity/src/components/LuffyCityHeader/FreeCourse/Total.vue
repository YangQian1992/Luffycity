<template>
  <div>
    <section v-for="item in courseList">
      <div class="head"><img src="../../../assets/sketch-small.576677d.png" alt="" class="bacimg"> <b class="mask" id="mask" style="background: #56CBC4"></b>
        <p>{{item.course__cname}}</p>
      </div>
      <div class="content"><p class="p1">{{item.course__coursedetail__course_overview}}</p>
        <div class="div1">
          <div class="div2"><img
            ...
            alt="">
            <span class="span1">{{item.course__coursedetail__number_of_students_studied}}</span>
            <span class="span2">{{item.course__coursedetail__course_difficulty}}</span>
          </div>
          <div class="div3"><span class="span4">￥9.9</span></div>
        </div>
      </div>
    </section>
  </div>
...
<script>
  export default {
    ...
    mounted() {
      let that = this;
      this.$axios.request({
        url: 'http://127.0.0.1:8000/luffycity_all_course/',
        method: 'get',
      }).then(function (data) {
...
<style scoped>
  ...
  section .content .div1 img {
    width: 11px;
    height: 12px;
  }

  section .content .div1 .div2 span {
    font-size: 13px;
    color: #666;
    letter-spacing: .29px;
  }

  section .content .div1 .span1 {
    margin-left: 5px;
    margin-right: 5px;
  }

  section .content .div1 span {
    display: inline-block;
  }

  section .content .div1 .div3 .span4 {
    font-size: 14px;
    color: #fa6240;
    letter-spacing: .32px;
  }
